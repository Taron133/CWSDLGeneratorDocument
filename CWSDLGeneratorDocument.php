<?php
/**
 * CWsdlGeneratorDocument class file.
 *
 * @author Taron <taron133@yandex.ru>
...
/**
 * CWsdlGeneratorDocument generates the WSDL for a given service class.
 *
 ...
 *
 * CWsdlGeneratorDocument recognizes the following primitive types (case-sensitive) in
 ...
 * CWsdlGeneratorDocument will look for its property declarations. Only public properties
 ...
 *
 * CWsdlGeneratorDocument recognizes the array type with the following format:
 * <pre>
 * typeName[]: maps to tns:typeNameArray NOT SUPPORT IN document/literal
 * </pre>
 *
 ...
 *   * @return string some array {minOccurs=0, maxOccurs=unbounded}// in document/literal need use maxOccurs=unbounded
 *   * @soap
 *   * /
 ...
 * a primitive type, CWsdlGeneratorDocument will look further to find the definition of 'Member'.
 *
...
class CWsdlGeneratorDocument
{
	...
	protected static $typeMap=array(
		...
		//'array'=>'soap-enc:Array',  // in document/literal need use maxOccurs=unbounded
		'object'=>'xsd:struct',
		'mixed'=>'xsd:anyType',
	);
	...
	*/
	...
	protected function processMethod($method)
	{
		...
		$this->messages[$methodName.'In'] = array('parameters' => array($methodName)); // $message
	...
	protected function processType($type)
	{
		...
		}*/ // in document/literal need use maxOccurs=unbounded
		else
...
<definitions name=\"{$this->serviceName}\" targetNamespace=\"{$this->namespace}\"
	...
	protected function addTypes($dom)
	{
        ...
        $schema->setAttribute('xmlns:tns',$this->namespace);
		...
		foreach($this->types as $phpType=>$xmlType)
		{
			...
			if(is_string($xmlType))
			{
				...
				$attribute->setAttribute('arrayType',substr($xmlType,0,strlen($xmlType)-5).'[]');
				...
				$attribute->setAttribute('arrayType',$arrayType);
        ...
        // elements of methods
        foreach($this->elements as $phpType=>$xmlType)
        {
            $mainElement=$dom->createElement('xsd:element');
            $mainElement->setAttribute('name', $phpType);
            $mainElement->setAttribute('type', 'tns:'.$phpType);
            $schema->appendChild($mainElement);
            ...
            foreach($xmlType as $name=>$type)
            {
                ...
                //
                $attributes=$this->getWsdlElementAttributes($type[1]);
                if(!is_null($attributes['minOccurs']))
                    $element->setAttribute('minOccurs',$attributes['minOccurs']);
                if(!is_null($attributes['maxOccurs']))
                    $element->setAttribute('maxOccurs',$attributes['maxOccurs']);
                if(!is_null($attributes['nillable']))
                    $element->setAttribute('nillable',$attributes['nillable']);
                $element->setAttribute('name', $name);
                $element->setAttribute('type', $type[0]);
                $all->appendChild($element);
            }
		...
		foreach($this->messages as $name=>$message)
		{
			...
			foreach($this->messages[$name] as $partName=>$part)
			{
				if(is_array($part))
				{
                    ...
                    $partElement->setAttribute('element','tns:'.$part[0]);
	...
	protected function createPortElement($dom,$name,$doc)
	{
		...
		$operation->appendChild($dom->createElement('documentation',$doc));
	...
	protected function addBindings($dom)
	{
		...
		//$soapBinding->setAttribute('style','rpc'); //taron
	...
	protected function createOperationElement($dom,$name)
	{
		...
		//$soapOperation->setAttribute('style','rpc'); //taron
        $soapOperation->setAttribute('style','document');

		$input=$dom->createElement('input');
        $input->setAttribute('name',$name);
		$output=$dom->createElement('output');
        $input->setAttribute('name',$name.'Response');
